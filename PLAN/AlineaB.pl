...
calcula_custo(LC,Custo,LEcarregamentos):-
    ...
    custo(LCcompleto,LPT,Autonomia,Custo,LEcarregamentos). % Ele começa com a autonomia maxima
...
custo([C1,C2|LC],[PT|LPT],Autonomia,Custo,LEcarregamentos):-
    ...
    Total is TT + CC,
    calcula_tempo_energia(Tempo,Energia,Total,PT,R,R1), % Aqui calcula o tempo e a energia entre 2 armazéns consoante o peso
    ...
    ((TCarga>TempoDescarga,!,TGasto is TCarga+TempoExtraAdicionado);TGasto is TempoDescarga+TempoExtraAdicionado)); % Aqui, ver qual dos tempos é maior e considerar esse
...
atualiza(LEPerm,Tempo,LEcarregamentos):-
    ...
    write('Tempo='),write(Tempo), write(' '),write(LEPerm), write(' com carregamentos em '),write(LEcarregamentos),nl)
    ;true).