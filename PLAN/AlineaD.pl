...
todos_caminhos(L,RS,RL):-
    getPaths(L,RL1),
    findall(RS,permutation(RL1,RS),RL).
...
%c calcula_custo
calcula_custo(LC,Custo,LEcarregamentos):-
    soma_pesos(LC,LP,_),
    carateristicasCam(_,Tara,_,Autonomia,_,_),
    acrescenta_tara(Tara,LP,LPT),
    armazemPrincipal(_,Cin),
    append([Cin|LC],[Cin],LCcompleto),
    custo(LCcompleto,LPT,Autonomia,Custo,LEcarregamentos),!. % Ele come�a com a autonomia maxima

is_empty(List):- not(member(_,List)).

custo([_],[],_,0,[]).
custo([C1,C2|LC],[PT|LPT],Autonomia,Custo,LEcarregamentos):-
    %write("Autonomia no armazem "),write(C1),write(": "),write(Autonomia),nl,
    (dadosCam_t_e_ta(_,C1,C2,Tempo,Energia,TempoExtra);dadosCam_t_e_ta(_,C2,C1,Tempo,Energia,TempoExtra)),
    %write("Tempo Extra: "),write(TempoExtra),nl,
    carateristicasCam(_,TT,CC,CTB,_,TR),
    Total is TT + CC,
    ...
    custo([C2|LC],LPT,A1,Custo1,LEcarregou),
...
run:-
    ...
    findall(Armazem,entrega(_,_,_,Armazem,_,_),LEsc),
    prefix(LEsc, 5, LEscFirstFive),
    permutation(LEscFirstFive,LEPerm),
    calcula_custo(LEPerm,Tempo,LEcarregamentos),
    atualiza(LEPerm,Tempo,LEcarregamentos),
    fail.

atualiza(LEPerm,Tempo,LEcarregamentos):-
    tempo_min(_,_,TempoMin),
    ((Tempo<TempoMin,!,retract(tempo_min(_,_,_)),
    assertz(tempo_min(LEPerm,LEcarregamentos,Tempo)))
    ;true).

% ------------------------------------------

solutionTime(Le, T, TSol):- get_time(Ti), 
    seq_tempo_min(Le,_,T),
    get_time(Tf),
    TSol is Tf - Ti.
...
lista_custo([],_,[]):-!.
lista_custo(LA,I,[A|LF]):- armazem_mais_prox(LA,I,_,A), !,delete(LA,A,LI), lista_custo(LI,A,LF).

heuristica_tempo(Lc, Custo, LCarregamentos):- 
lista_custo(Lc, 5, Lf),
calcula_custo(Lf, Custo, LCarregamentos).

heuristica_tempo_solution_time(La, T, TSol):- get_time(Ti),
    heuristica_tempo(La, T, _),
    get_time(Tf),
    TSol is Tf - Ti.

maior_massa_armazem([X],Max,A):- entrega(_,_,T, X,_ ,_ ), !, Max is T, A is X.
maior_massa_armazem([X|LA],Max,A):-
    entrega(_,_,T, X,_ ,_ ),
    maior_massa_armazem(LA,Max1,A1), ((T>Max1,!,Max is T, A is X); Max is Max1,A is A1).

lista_massa([],[]):-!.
lista_massa(LA,[A|LF]):- maior_massa_armazem(LA,_,A),!, delete(LA,A,LI), lista_massa(LI,LF).

heuristica_massa(LC,Custo,LCarregamentos):-
    lista_massa(LC,LF),
    calcula_custo(LF,Custo,LCarregamentos).

heuristica_massa_solution_time(La, T, TSol):- get_time(Ti),
    heuristica_massa(La, T, _),
    get_time(Tf),
    TSol is Tf - Ti.



maior_massa_tempo([X],I,Max,A):-dadosCam_t_e_ta(_, I, X, T,_ ,_ ), entrega(_,_,Y, X,_ ,_ ), Max is T*Y, A is X,! .
maior_massa_tempo([X|LA], I,Max,A):-
    dadosCam_t_e_ta(_, I, X, T,_ ,_ ),entrega(_,_,Y, X,_ ,_ ),
    maior_massa_tempo(LA,I,Max1,A1), ((T*Y<Max1,!,Max is T*Y, A is X); Max is Max1,A is A1).



lista_custo_massa([],_,[]):-!.
lista_custo_massa(LA,I,[A|LF]):- maior_massa_tempo(LA,I,_,A),!, delete(LA,A,LI), lista_custo_massa(LI,A,LF).


heuristica_massa_tempo(LC,Custo,LCarregamentos):-
    lista_custo_massa(LC,5,LF),
    calcula_custo(LF,Custo,LCarregamentos).

heuristica_massa_tempo_solution_time(La, T, TSol):- get_time(Ti),
    heuristica_massa_tempo(La, T, _),
    get_time(Tf),
    TSol is Tf - Ti.
