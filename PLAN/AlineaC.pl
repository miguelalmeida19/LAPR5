...
energia_disponivel(EnergiaDispo,EnergiaGast,BateriaTotal,TempoExtra,R,TempoExtraAdicionado):-
    write(EnergiaDispo),write(" - "),write(EnergiaGast),write(") * "),
...
tempo_carregamento(C1,R1,BateriaTotal,TRecarga,TempoExtra,TCarga,TempoExtraAdicionado):-
    write("tempo_carga["),write(C1),write("] = ("),
    carregar_oitenta(BateriaTotal,Bateria),
    energia_disponivel(Bateria,R1,BateriaTotal,TempoExtra,R,TempoExtraAdicionado),
    write(TRecarga),write("/48 = "),
...
calcula_custo(LC,Custo,LEcarregamentos):-
    ...
    custo(LCcompleto,LPT,Autonomia,Custo,LEcarregamentos). % Ele comeca com a autonomia maxima

is_empty(List):- not(member(_,List)).

custo([_],[],_,0,[]).
custo([C1,C2|LC],[PT|LPT],Autonomia,Custo,LEcarregamentos):-
    write("tempo["),write(C1),write("->"),write(C2),write("]="),
    ...
    Total is TT + CC,
    write(PT),write(" / "),write(Total),write(" = "),
    ...
    ((Autonomia<R1,!,tempo_carregamento(C1,Autonomia,CTB,TR,TempoExtra,TCarga,TempoExtraAdicionado),
