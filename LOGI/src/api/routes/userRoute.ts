...
export default (app: Router) => {
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const userOrError = await authServiceInstance.SignUp(body as IUserDTO);
  ...
  route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
      ...
      try {
        ...
        const authServiceInstance = Container.get(AuthService);
        const result = await authServiceInstance.Login(email);

        if (result.isFailure) {
          logger.debug(result.errorValue())
          return res.status(401).send(result.errorValue());
        }
  ...
  route.post('/cancel',
    ...
    (req, res, next) => ctrl.cancelAccount(req, res, next) );
