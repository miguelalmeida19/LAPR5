...
export default class UserService implements IUserService{
  ...
  public async cancelAccount(email: string): Promise<Result<IUserDTO>> {
    try {
      ...
      if (user === null) {
        ...
        const firstNameHash = crypto.createHash('sha256').update(user.firstName).digest('hex');
        const lastNameHash = crypto.createHash('sha256').update(user.lastName).digest('hex');
        user.email = UserEmail.create(emailHash).getValue();
        user.firstName = firstNameHash;
        user.lastName = lastNameHash;
  ...
  public async Login(email: string): Promise<Result<{ userDTO: IUserDTO, token: string }>> {
  ...
  public async getAll(): Promise<Result<IUserDTO[]>> {
    const user_list = await this.userRepo.findAll();

    if (user_list === null) {
      return Result.fail<IUserDTO[]>("Can't get users");
    }

    // Create an array of promises that will resolve to the role names for each user
    const rolePromises = user_list.map(async user => {
      const role = await this.roleRepo.findByDomainId(String(user.role));
      return role.props.name;
    });

    // Wait for all the role name promises to resolve
    const roleNames = await Promise.all(rolePromises);

    // Map the role names to the user DTOs
    const userDTOs = user_list.map((user, index) => {
      return UserMap.toDTORole(user, roleNames[index]);
    });
